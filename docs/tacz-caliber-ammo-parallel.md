...
**PC-3 · 内容(口径 + 整套弹药 + 枪字段 + 配方)**  ☑ owner:Agent 2 产出: 打通并 runServer 核验内容加载链 —— `datagen`(ModDataGen+CaliberAmmoDataProvider) → `runData` → `src/generated/resources` 数据包 → TacZ `CommonDataManager` 扫 `data/tacz_caliber_ammo/index/ammo/<口径>/<型号>` → 本 mod `CommonDataManagerMixin` 读 caliber+伤害 → `CaliberManager`（**内置内容走数据包, 无需 gun pack 注册**）。已完成(现版, **CSV 全量生成**): `CaliberAmmoDataProvider` 全量读 `docs/tarkov_ammo_stats.csv` → **37 口径定义 + 207 弹种 JSON**(caliber + baseDamage/armorIgnore/headShotMultiplier/pierce + 弹道 recoil/accuracy/speed/pelletCount); 弹药 name/abbr lang 全量(en/zh 各 200+ 键)。**枪→口径: 全 54 枪经 NATIVE_MAP Tier-3 自动解析**(19 个原生口径全在 NATIVE_MAP, CR-4 已核验 glock/kar98/deagle 等), `modify_gun_caliber` 4 枪 demo 冗余保留。runData 生成 172 ammo(抽查 m855 speed=922/pierce1 对上 CSV)。**已完成收尾(进入维护阶段)**: (a) recipes —— buildRecipe 生成 tacz:gun_smith_table_crafting 配方(铜/铁粒/火药按穿甲 pen 缩放, 高穿甲=更多材料; 分类映射 group: 手枪|PDW=pd_cartridges, 步枪=ifp_rifle_cartridges, 霰弹=shotgun_shells, 榴弹=explosives), runData 出 207 配方(抽查 m2_ap pen62 = 25 铜+3 火药+5 铁粒); (b) 7 个有原生枪的口径已补齐(d22_wmr/d500_magnum/5_8x42/d30_06/7_92x57_mauser/d45_70/72_5, 共 28 弹种, CSV 补录数值 + lang 补 en/zh 各 56 键 + 复用 tacz 原生 display 图标, 7 个 display 均存在); (c) tags —— 经查 TacZ 的 tag 仅玩法用(伤害类型/穿透白名单等), 与弹药分组或合成无关, 无需新增; (d) 数值平衡以 CSV 派生为基线。3 个无原生枪的口径(12_7x108/30x29/40_vog_25)由并行会话加入 CAL_ID 备用, 无枪引用不影响可玩。游戏内配方获取/换弹/伤害/tooltip 观测待手动 runClient(无头无法核验)。
...
| CR-10 | 2026-07-15 | 用户+Agent | **枪/子弹 tooltip 配色 + 精简**: (1) 枪 tooltip 普通行(口径/列名/类型/移速/desc/pack)改深灰 `0xAAAAAA`(常量 LABEL), 仅 容量值 ammoCountText(30/30) 与 子弹明细 保持白 `0xFFFFFF`(VALUE); gunType/weight/packInfo 带 TacZ 青/红/蓝 baked 样式(传色不覆盖)→ 用 `grayText(c)=literal(c.getString()).withStyle(DARK_GRAY)` 重建再画。(2) **移除枪 tooltip 的 经验等级(levelInfo) + "按 Z 改造"(UPGRADES_TIP)**; renderText 删两块, getHeight BASE_INFO -10→**-20**、新增 UPGRADES_TIP **-14**; 去 levelInfo/tips 两 @Shadow。(3) **子弹(弹药)物品 tooltip**: TooltipHandler.appendAmmo 每行改 `valueLine`(标签深灰 DARK_GRAY / 数值白 WHITE, 值包 `literal(v).withStyle(WHITE)`), 并 `lines.addAll(min(1,size),out)` **插到 index 1(物品名下第一行)**; gunCaliberLine 样式 GRAY→DARK_GRAY。 | 用户追加(枪 tooltip 深灰+容量/装填白; 删经验/按Z; 子弹 tooltip 深灰+数值白+移到第一行); `:1.20.1-forge:compileJava` 绿(仅 ResourceLocation 弃用告警), API 全解析(ChatFormatting.DARK_GRAY/WHITE, Component.getString/withStyle/translatable/literal, List.addAll(index,coll))。**游戏内(深灰/白配色, 无经验/按Z 行, 子弹 tooltip 首行位置+配色)未核验, 留 runClient**。 |
| CR-11 | 2026-07-15 | 用户+Agent | **tooltip 配色再调(现版, 覆盖 CR-10 配色)**: 标签 深灰→**浅灰 `0xAAAAAA`**(LABEL); 值→**橙 GOLD**(口径名/子弹 abbr/容量上限)与 **白 WHITE**(子弹数量/容量当前数, 按 `/` 切 ammoCountText.getString())。**desc 不改**(原 0xAAAAAA)、**gunType 类型名(青)/weight 移速(红)/packInfo(蓝) 保留 TacZ 原样式**(删 grayText, 直接画 raw shadow; gunType 传 LABEL 令"类型："浅灰、类型名仍青)。分色靠 `literal(a).withStyle(A).append(literal(b).withStyle(B))`——因 drawInBatch 的 color param 只染无显式样式文本, 组件 baked 样式覆盖 param。`loadedAmmoLines` 改返 `List<Component>`(数量白+abbr橙), 新增 `capacityText`(当前白+/上限橙), 删 grayText/VALUE 常量, LABEL 改 0xAAAAAA。子弹物品 tooltip: `valueLine` 标签 GRAY/值 GOLD, `gunCaliberLine` 名 GOLD/标签 GRAY, `noCaliberLine` GRAY。 | 用户追加(标签浅灰值橙; 数量白/abbr橙; 容量当前白/上限橙; desc/移速/枪种保留; 子弹标签浅灰值橙); `:1.20.1-forge:compileJava` 绿(仅 ResourceLocation 弃用告警), API 全解析(ChatFormatting.GOLD/GRAY/WHITE, Component.append/withStyle/getString, Font.width(FormattedText))。**游戏内(浅灰标签/橙值/白数量/容量分色/枪种青/移速红保留)未核验, 留 runClient**。 |
| CR-12 | 2026-07-16 | 用户+Agent | **弹药 tooltip 后坐力行颜色反向**: `TooltipHandler.signedValueLine` 原对 后坐力/精度 都"正=绿/负=红/0=白"。后坐力 `正 recoil%` = 更多后坐力 = **坏**, 却显绿(误当好)。给 `signedValueLine` 加 `higherIsWorse` 参: 后坐力 `true`(按 `-v` 判好坏 => 正红/负绿), 精度 `false`(正绿/负红) 不变。**只改颜色, 显示的带符号数字不变**。(先核对: 后坐力**机制**方向对 —— CameraSetupEventMixin `×(1+recoil%/100)` + 反编译 GunRecoil `values=base*modifier`, 只是 tooltip 着色反了。) | 用户澄清"我说的是 tooltip 的颜色"; `:1.20.1-forge:compileJava` 绿(仅 ResourceLocation 弃用告警)。**游戏内(后坐力正值显红/负值显绿, 精度不变)未核验, 留 runClient**。 |
| CR-13 | 2026-07-16 | 用户+Agent | **多头弹药(霰弹)伤害修复**: TacZ `ScriptAPI.shootOnce` 每颗弹丸 `applyShotgunDamageSpread(bulletAmount)` 设 `damageModifier=1/bulletAmount`(假设枪持总伤害), 与本 mod"弹药持每颗伤害"冲突→N 颗合计只 = baseDamage(观感"多颗命中只造成一次伤害")。改 `EntityKineticBulletMixin` 加 `@Inject(applyShotgunDamageSpread, HEAD, cancellable, remap=false)`: `bulletCount>1 && getAmmoProfile(ammoId)!=null` 时 `ci.cancel()` 跳过均摊→每颗打满 baseDamage, N 颗即 N×baseDamage; 未配置档弹药保留 TacZ 均摊。物理散布(doBulletSpread)不受影响; TacZ 本就 `f_19802_=0` 绕无敌帧故每颗都结算。 | 用户报 bug(多弹丸命中同目标只一次伤害); CFR 核验伤害链(getDamage×damageModifier, tacAttackEntity 重置无敌帧, shootOnce 循环调 applyShotgunDamageSpread); `:1.20.1-forge:compileJava` 绿(仅 ResourceLocation 弃用告警)。**游戏内(多头每颗打满、总伤=N×base、单发弹不受影响)未核验, 留 runClient 实测**。 |
